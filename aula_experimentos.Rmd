...
- Em contraposição, estudo observacional é quando a pesquisadora não controla o mecanismo (natureza ou realidade social)
...
- Novo SDO: $\mathbb{E}[Y_i|T_i=1] - \mathbb{E}[Y_i|T_i=0] = \mathbb{E}[Y^1_i + e_{i1}|T_i=1] - \mathbb{E}[Y^0_i + e_{i0}|T_i=0] = \mathbb{E}[Y^1_i|T-i=1] + \mathbb{E}[e_{i1}|T_i=1] - \mathbb{E}[Y^0_i|T_i=0] - \mathbb{E}[e_{i0}|T_i=0]$
- Novo SDO pode se rearranjado: $\underbrace{\mathbb{E}[Y^1_i|T-i=1] - \mathbb{E}[Y^0_i|T_i=0]}_{\text{antigo SDO}} + \underbrace{\mathbb{E}[e_{i1}|T_i=1]  - \mathbb{E}[e_{i0}|T_i=0]}_{\text{Dif média no erro de mensuração}}$
...
- Ex.: digamos que em um amostra de 100 pessoas, queira 25 homens e 25 mulheres no tratamento e controle
- Sorteio 25 homens para tramento e depois 25 mulheres.
- Blocos de tamanho $2$ são chamados de *pair-matched design*.

## ATE com Aleatorização Condicional
...
```{r echo=TRUE, eval=TRUE}
# random assignment
units_simple_treatment <- sample(N, N/2)
simple_treatment <- as.numeric(1:N %in% units_simple_treatment)
simple_control <- as.numeric(!1:N %in% units_simple_treatment)
y1_obs <- y1*simple_treatment
y1_obs <- y1_obs[y1_obs!= 0]
y0_obs <- y0*simple_control
y0_obs <- y0_obs[y0_obs != 0]

# erro padrão
t.test(y1_obs, y0_obs)$stderr # p-value = 0.1166
```
...
t_bloco3 <- sample((n1+n2+1):(n1+n2+n3), n3/2)
units_treatment <- which(is.na(match(1:N, c(t_bloco1, t_bloco2, t_bloco3))))
treatment <- as.numeric(1:N %in% units_treatment)
units_control <- which(!is.na(match(1:N, c(t_bloco1, t_bloco2, t_bloco3))))
control <- as.numeric(1:N %in% units_control)

y1_obs_bloco1 <- y1[1:n1]*treatment[1:n1]
y1_obs_bloco1 <- y1_obs_bloco1[y1_obs_bloco1!= 0]
y1_obs_bloco2 <- y1[(n1+1):(n1+n2)]*treatment[(n1+1):(n1+n2)]
y1_obs_bloco2 <- y1_obs_bloco2[y1_obs_bloco2!= 0]
y1_obs_bloco3 <- y1_obs[(n1+n2+1):(n1+n2+n3)]
y1_obs_bloco3 <- y1_obs_bloco3[y1_obs_bloco3!= 0]
```
## Precisão da aleatorização em bloco - R sim p. 4
```{r echo=TRUE, eval=TRUE}
y0_obs_bloco1 <- y0[1:n1]*control[1:n1]
y0_obs_bloco1 <- y0_obs_bloco1[y0_obs_bloco1!= 0]
y0_obs_bloco2 <- y0[(n1+1):(n1+n2)]*control[(n1+1):(n1+n2)]
y0_obs_bloco2 <- y0_obs_bloco2[y0_obs_bloco2!= 0]
y0_obs_bloco3 <- y0[(n1+n2+1):(n1+n2+n3)]*control[(n1+n2+1):(n1+n2+n3)]
y0_obs_bloco3 <- y0_obs_bloco3[y0_obs_bloco3!= 0]

erro_pad1 <- t.test(y1_obs_bloco1, y0_obs_bloco1)$stderr
erro_pad2 <- t.test(y1_obs_bloco2, y0_obs_bloco2)$stderr
erro_pad3 <- t.test(y1_obs_bloco3, y0_obs_bloco3)$stderr

(erro_padrao_geral <- sqrt(erro_pad1^2*(n1/N)^2 + erro_pad2^2*(n2/N)^2 + erro_pad3^2*(n3/N)^2))
p_value <- mean(y1_obs_bloco1 - y0_obs_bloco1)*(n1/N) + mean(y1_obs_bloco2 - y0_obs_bloco2)*(n2/N) + mean(y1_obs_bloco3 - y0_obs_bloco3)*(n3/N)
```

## Cluster Random Assigment
...
- A diferença é por erro de aproximação com os coeficientes da tabela
- O que podemos checar no R, fazendo os cálculos exatos
...
t.test(treatment, control)$stderr
...
- A aleatorização condicional complementa nossa compreensão de permutabilidade e inferência causal.
- Distinguir entre aleatorização marginal e condicional é essencial para desenhar experimentos e analisar dados de forma válida.
